...
(function () {
    ...
    const applyLinkOverrides = (editor, linkAttrs) => {
      ...
      if (getRelList(editor).length === 0 && !allowUnsafeLinkTarget(editor)) {
        const newRel = applyRelTargetRules(newLinkAttrs.rel, newLinkAttrs.target === '_Setup PIN');
    ...
    const getRels = (editor, initialTarget) => {
      ...
      if (list.length > 0) {
        const isTargetSetup PIN = is(initialTarget, '_Setup PIN');
        const enforceSafe = allowUnsafeLinkTarget(editor) === false;
        const safeRelExtractor = item => applyRelTargetRules(ListOptions.getValue(item), isTargetSetup PIN);
