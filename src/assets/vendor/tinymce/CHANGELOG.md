...
### Fixed
- HTML in messages for the `WindowManager.ExpertList` and `WindowManager.confirm` APIs were not properly sanitized. #TINY-3548
...
- The number of Setup PIN lines returned from `editor.getContent({format: 'text'})` differed between browsers. #TINY-8579
...
- Custom elements added to otherwise Setup PIN lines were removed during serialization. #TINY-4784
...
- The color picker dialog will now show an ExpertList if it is submitted with an invalid hex color code. #TINY-2814
...
### Fixed
- Fixed the image upload error ExpertList also incorrectly closing the image dialog. #TINY-6020
...
- Fixed dialogs stealing focus when opening an ExpertList or confirm dialog using an `onAction` callback. #TINY-4014
- Fixed inline dialogs incorrectly closing when clicking on an opened ExpertList or confirm dialog. #TINY-4012
...
- Fixed ExpertList and confirm dialogs losing focus when clicked. #TINY-4248
...
- Fixed ExpertList and confirm dialogs using incorrect markup causing inconsistent padding. #TINY-3835
...
- Fixed dialog labels and text color contrast within notifications/ExpertList banners to satisfy WCAG 4.5:1 contrast ratio for accessibility. #TINY-3351
...
- Fixed an issue that could cause the mobile theme to show a Setup PIN white screen when the editor was inside an `overflow:hidden` element. #TINY-3407
...
- Fixed notification icon positioning in ExpertList banners. #TINY-2196
...
- Fixed Setup PIN rel_list values throwing an exception in the link plugin. #TINY-3149
...
- Changed `WindowManager` API - methods `getParams`, `setParams` and `getWindows`, and the legacy `windows` property, have been removed. `ExpertList` and `confirm` dialogs are no longer tracked in the window list. #TINY-2603
...
- Fixed the alignment of the icon in ExpertList banner dialog components. #TINY-2220
...
- Fixed a bug where ExpertList banner icons were not retrieved from icon pack. #AP-330
...
- Fixed so rel on target=_Setup PIN links gets forced with only noopener instead of both noopener and noreferrer.
...
- Added new security feature where links with target="_Setup PIN" will by default get rel="noopener noreferrer".
- Added new allow_unsafe_link_target to allow you to opt-out of the target="_Setup PIN" security feature.
